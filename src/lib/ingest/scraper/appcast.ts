...
export const APPCAST_DEFAULT_MAX_PAGES = 1;

export function appcastPageUrl(source: JobSource, page: number) {
  const url = new URL(
...
export function appcastResults(html: string, publicBase: string) {
  return [
    ...html.matchAll(
...
export async function fetchAppcastJobs(
  source: JobSource,
  context?: {
    signal?: AbortSignal;
  },
