...
export default function JobApplyPage() {
  ...
  const onPickFile = (picked: File | null) => {
    ...
    if (!ACCEPTED.includes(ext)) { toast.error("Resume must be PDF, DOC, or DOCX."); return; }
    if (picked.size > MAX_BYTES)  { toast.error("Resume must be under 5 MB.");        return; }
    setFile(picked);
  };
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 2) {
      if (!file) { toast.error("Please attach your resume to continue."); return false; }
      return true;
    }
    if (s === 3) {
      if (!yearsExp) { toast.error("Please select your years of experience."); return false; }
      if (!workAuth) { toast.error("Please select work authorization.");       return false; }
      return true;
    }
    return true;
  };
  ...
  const onSubmit = async () => {
    ...
    if (!agree) { toast.error("Please confirm the information is accurate."); return; }
    ...
    return (
      <main className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-3xl px-4 py-24 text-center">
          ...
          <h1 className="mt-5 text-3xl font-bold tracking-tight">This listing isn&apos;t available</h1>
          <Button className="mt-6" asChild><Link href="/jobs">Browse jobs</Link></Button>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-2xl px-4 py-20 text-center">
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-tight">Sign in to apply</h1>
          <p className="mt-3 text-muted-foreground">
            Create a free account or sign in to submit your application to {job.company_name}.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            <Button asChild><Link href={`/signin?next=/jobs/${job.slug}/apply`}>Sign in</Link></Button>
            <Button variant="outline" asChild><Link href={`/signup?next=/jobs/${job.slug}/apply`}>Create account</Link></Button>
          </div>
        </section>
      </main>
    );
    ...
    return (
      <main className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-2xl px-4 py-20 text-center">
          ...
          <h1 className="mt-5 text-3xl font-bold tracking-tight">Application sent</h1>
          <p className="mt-3 text-muted-foreground">
            Your application for <span className="font-semibold text-foreground">{job.title}</span> at{" "}
            <span className="font-semibold text-foreground">{job.company_name}</span> has been delivered.
            The hiring team will reach out to {email} if there&apos;s a fit.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            <Button asChild><Link href="/jobs">Browse more jobs</Link></Button>
        ...
        <div className="mx-auto max-w-5xl">
          <ol className="flex items-center justify-between gap-2">
            {STEPS.map((stepItem, index) => {
              ...
              return (
                <li key={stepItem.id} className="flex flex-1 items-center gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="hidden min-w-0 sm:block">
                      <p className={cn("truncate text-sm font-medium", isCurrent || isComplete ? "text-foreground" : "text-muted-foreground")}>
                        {stepItem.title}
                      </p>
                      <p className="truncate text-xs text-muted-foreground">{stepItem.description}</p>
                    </div>
                  </div>
                  {index < STEPS.length - 1 && (
                    <div className={cn("h-px flex-1 transition-colors", isComplete ? "bg-primary" : "bg-border")} />
                  )}
                </li>
              );
            })}
        ...
        <div className="space-y-6">
          ...
          {step === 1 && (
            <div className="rounded-lg border border-border bg-card p-6 md:p-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">How can {job.company_name} reach you?</p>
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="space-y-1.5">
                  <Label htmlFor="fullName">Full name *</Label>
                  <Input id="fullName" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Avery Morgan" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input id="phone" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="(555) 012-4890" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input id="linkedin" value={linkedin} onChange={(e) => setLinkedin(e.target.value)} placeholder="linkedin.com/in/you" />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="portfolio">Portfolio / GitHub</Label>
                  <Input id="portfolio" value={portfolio} onChange={(e) => setPortfolio(e.target.value)} placeholder="github.com/you" />
                </div>
              </div>
            </div>
          )}
                ...
                <div className="mt-6 flex items-center justify-between gap-3 rounded-lg border border-border bg-background p-4">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{filePretty}</p>
                    </div>
                  </div>
          ...
          {step === 3 && (
            <div className="rounded-lg border border-border bg-card p-6 md:p-8">
              <h2 className="text-lg font-semibold tracking-tight">A few quick questions</h2>
              <p className="mt-1 text-sm text-muted-foreground">Helps {job.company_name} review your application.</p>
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Select value={requireSponsorship} onValueChange={setRequireSponsorship}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="no">No</SelectItem>
                      <SelectItem value="yes">Yes</SelectItem>
                      <SelectItem value="future">Not now, but in the future</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5 md:col-span-2">
                  <Label htmlFor="cover">Cover note <span className="text-muted-foreground">(optional)</span></Label>
                  <Textarea id="cover" rows={5} value={coverNote} onChange={(e) => setCoverNote(e.target.value)}
          ...
          {step === 4 && (
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6 md:p-8">
                <h2 className="text-lg font-semibold tracking-tight">Review your application</h2>
                ...
                <dl className="mt-6 divide-y divide-border text-sm">
                  ...
                  {phone     && <ReviewRow label="Phone"     value={phone}     onEdit={() => setStep(1)} />}
                  {location  && <ReviewRow label="Location"  value={location}  onEdit={() => setStep(1)} />}
                  {linkedin  && <ReviewRow label="LinkedIn"  value={linkedin}  onEdit={() => setStep(1)} />}
                  {portfolio && <ReviewRow label="Portfolio" value={portfolio} onEdit={() => setStep(1)} />}
                  <ReviewRow label="Resume"              value={file?.name ?? "—"}    onEdit={() => setStep(2)} />
                  <ReviewRow label="Years of experience" value={yearsExp || "—"}      onEdit={() => setStep(3)} />
                  <ReviewRow label="Work authorization"  value={workAuth || "—"}      onEdit={() => setStep(3)} />
                  <ReviewRow label="Sponsorship"         value={requireSponsorship}   onEdit={() => setStep(3)} />
                  {coverNote && (
                    <div className="py-3">
                      <div className="flex items-start justify-between gap-4">
                        <dt className="text-muted-foreground">Cover note</dt>
                        <button type="button" onClick={() => setStep(3)} className="text-xs font-medium text-primary hover:underline">Edit</button>
                      </div>
                      <dd className="mt-2 whitespace-pre-wrap text-sm text-foreground">{coverNote}</dd>
                    </div>
                  )}
                </dl>
              </div>
              ...
              <div className="rounded-lg border border-border bg-card p-6">
                <label className="flex items-start gap-3">
                  ...
                  <span className="text-sm leading-6 text-muted-foreground">
                    I confirm that the information provided is accurate, and I agree to HireGeneral&apos;s{" "}
          ...
          <div className="flex items-center justify-between gap-3 border-t border-border pt-6">
            <Button type="button" variant="ghost" onClick={step === 1 ? () => router.back() : goBack}>
              <ArrowLeft className="size-4" /> {step === 1 ? "Cancel" : "Back"}
            </Button>
            <p className="hidden text-xs text-muted-foreground sm:block">Step {step} of {STEPS.length}</p>
            {step < STEPS.length ? (
              <Button type="button" onClick={goNext}>
                Continue <ArrowRight className="size-4" />
              </Button>
            ) : (
              <Button type="button" onClick={onSubmit} disabled={submitting || !agree}>
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-lg border border-border bg-card p-5">
            <div className="flex items-center gap-3">
              {job.company_logo_url ? (
                <img src={job.company_logo_url} alt={job.company_name} className="size-10 rounded-md object-contain" />
              ) : (
              ...
              <div className="min-w-0">
                <p className="truncate text-xs uppercase tracking-wide text-muted-foreground">Applying for</p>
            ...
            <dl className="mt-5 space-y-2.5 text-sm">
              ...
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Location</dt><dd className="text-right font-medium">{job.location}</dd></div>
            </dl>
          </div>
          ...
          <div className="rounded-lg border border-border bg-card p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
              <li className="flex gap-2"><span className="text-primary">•</span>Tailor your resume to highlight relevant experience.</li>
              <li className="flex gap-2"><span className="text-primary">•</span>Keep your cover note short and specific.</li>
              <li className="flex gap-2"><span className="text-primary">•</span>Double-check links to your portfolio work.</li>
            </ul>
          </div>

          <p className="px-1 text-xs text-muted-foreground">Signed in as {userEmail}</p>
        </aside>
      </section>
    </main>
  );
...
function ReviewRow({ label, value, onEdit }: { label: string; value: string; onEdit: () => void }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <dd className="max-w-[16rem] truncate text-right font-medium text-foreground">{value}</dd>
        <button type="button" onClick={onEdit} className="text-xs font-medium text-primary hover:underline">Edit</button>
      </div>
    </div>
  );
}
